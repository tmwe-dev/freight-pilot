...
export default function MasterPageShell({ activePage, children, onPageChange }: MasterPageShellProps) {
  ...
  return (
    <div className="relative w-full h-screen bg-[#06060e] overflow-hidden flex flex-col">
      <motion.div className="absolute inset-0 pointer-events-none" animate={{ opacity: 1 }} transition={{ duration: 1 }}>
      ...
      <nav className="relative z-30 flex-shrink-0 border-b border-white/[0.06]">
        <div className="h-16 px-4 flex items-center">
          <motion.div className="flex items-center gap-2.5 mr-6 pr-6 border-r border-white/[0.08] cursor-default" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }}>
          ...
          <div className="flex items-center gap-1 flex-1">
            {PAGES.map((page, i) => {
              ...
              return (
                <motion.button key={page.id} onClick={() => onPageChange(page.id)} onMouseEnter={() => setHoveredTab(page.id)} onMouseLeave={() => setHoveredTab(null)} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06, duration: 0.4, ease: "easeOut" }} className={cn("relative flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-semibold tracking-wide transition-all duration-300 outline-none", isActive ? "text-white" : "text-white/35")} style={{ backgroundColor: isActive ? page.bg : isHovered ? page.hoverBg : "transparent" }}>
                  <motion.div animate={{ scale: isActive ? 1.1 : isHovered ? 1.2 : 1, rotate: isHovered && !isActive ? 8 : 0 }} transition={{ type: "spring", stiffness: 400, damping: 20 }}>
                    <Icon className={cn("w-4 h-4 transition-colors duration-300", isActive ? page.iconColor : isHovered ? page.iconColor : "text-white/30")} />
                  </motion.div>
                  <span className={cn("transition-colors duration-300", isHovered && !isActive && "text-white/70")}>{page.label}</span>
                  {isActive && (<motion.div layoutId="activeTabLine" className={cn("absolute -bottom-[1px] left-3 right-3 h-[2px] rounded-full bg-gradient-to-r", page.gradient)} transition={{ type: "spring", stiffness: 500, damping: 35 }} />)}
                  <AnimatePresence>
                    {isHovered && !isActive && (<motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.2 }} className="absolute inset-0 rounded-xl pointer-events-none" style={{ boxShadow: "inset 0 0 20px " + page.glow.replace("0.4", "0.08") + ", 0 0 15px " + page.glow.replace("0.4", "0.06") }} />)}
                  </AnimatePresence>
                </motion.button>
              );
            })}
